function initialize(mainPrice)
    {
      ...
      {
        var xc6=window.matchMedia("(min-width: 992px)");
        if (xc6.matches)
        {
      ...
      var price_us=["21.00","30.00","45.00","52.00","55.00","60.00","65.00","70.00","71.00","75.00","75.00","77.00","110.00","150.00"]; 
      var price_uk=["16.20","23.10","34.70","40.00","42.40","46.20","50.10","53.90","54.70","57.80","57.80","59.30","84.70","115.50"];
      var price_pk=["3,500","5,000 ","7,490","8,660","9,160","9,990","10,820","11,660","11,820","12,490","12,490","12,820","18,320","24,980"];   
      ...
      {    
        ...
        document.getElementById("main_curr").innerHTML="$";
        document.getElementById("main_price").innerHTML=price_us1[mainPrice-1];
      ...
      { 
        document.getElementById("cart_curr").innerHTML="£";
        document.getElementById("main_curr").innerHTML="£";
        document.getElementById("main_price").innerHTML=price_uk1[mainPrice-1];
      ...
      { 
        ...
        document.getElementById("main_curr").innerHTML="PKR";
        document.getElementById("main_price").innerHTML=price_pk1[mainPrice-1];
...
*/
     ...
     for (var q=lst; q<dt.length; q++)
     {
        ...
        {
          $(document).ready(function(){ $('#lp_error').hide(); });
          document.getElementById('lp_error').style.opacity="0";
        }
  ...
  function add_item(b1,n,num,index)
{
    ...
    var i=sessionStorage.getItem("user"), j=sessionStorage.getItem("psw"), li=i.length, lj=j.length;
    var st=i+li+"-"+j+lj+"/", dt=sessionStorage.getItem("data"), st1=st, nums, count=dt.search(st), lst=st.length+count, check1=0, st0=st;
     
    for (var w=lst; w<lst+36; w++)
  {
    if (dt.substr(w,1)==" " || dt.substr(w,1)=="*") { break; }
    if (dt.substr(w,1)==num) { check1=1; break; }
  }
  ...
  if (check1!=0)
  {
    $(document).ready(function(){ $('#lp_error').show(); });
      document.getElementById('lp_error').style.opacity="1";
      document.getElementById('lp_error').innerHTML="Item already added";
      $('#lp_error').show();
      setTimeout(function() { $('#lp_error').hide(); }, 1500);
      return;
  }

  if (sessionStorage.getItem("user")==null) { return; }
  if (document.getElementById('size-inside-text').innerHTML=="Select Size")
  {
    $(document).ready(function(){ $('#lp_error').show(); });
      document.getElementById('lp_error').style.opacity="1";
      document.getElementById('lp_error').innerHTML="No size selected";
      $('#lp_error').show();
      setTimeout(function() { $('#lp_error').hide(); }, 1500);
      return;
  }
    ...
    if (check1==0)
    {
      ...
      $(document).ready(function(){
        ...
        document.getElementById('cart').style.background="white";
        document.getElementById('cart').style.background="white";
        document.getElementById('cart').style.borderBottom="2px solid rgb(204, 204, 204)";
        document.getElementById('cart').style.borderLeft="2px solid rgb(204, 204, 204)";
      }); 
     var size_text=document.getElementById('size-inside-text').innerHTML;
     var quan_c=parseInt(document.getElementById('value').innerHTML), size_c=size_text.substr(0,1);
    
     if ((size_c=="s" || size_c=="m" || size_c=="l" || size_c=="S" || size_c=="M" || size_c=="L")  && (quan_c<10 && quan_c>0))
     {
    ...
    var user_string=i+li+"-"+j+lj+"/", first_indx=curr_dt.search(user_string);
    var curr_string="";
    ...
    var c_string=curr_string.includes("$");

    if (c_string==false)
    {
      var t_uk=p_uk*quan_c, t_pk=p_pk*quan_c, t_us=p_us*quan_c;
      t_us=t_us.toFixed(2);
      t_uk=t_uk.toFixed(2);      
      var curr_string1=curr_string+t_us+"($)"+t_uk+"(£)"+t_pk+"(PKR)";
      curr_dt=curr_dt.replace(curr_string,curr_string1);
      sessionStorage.setItem("curr_data",curr_dt);
      if (sessionStorage.getItem("fl")==1) 
      { document.getElementById('total').innerHTML=t_us; }
      else if (sessionStorage.getItem("fl")==2) 
      { document.getElementById('total').innerHTML=t_uk; }
      else if (sessionStorage.getItem("fl")==3) 
      { document.getElementById('total').innerHTML=t_pk; }
    }  
    ...
    var new_cr_us=old_cr_us1+(p_us*quan_c);
    var new_cr_uk=old_cr_uk1+(p_uk*quan_c);
    var new_cr_pk=old_cr_pk1+(p_pk*quan_c);
 ...
 /* CURRENCY CODE ENDS */
     ...
     var last_index=start_index+(user_st.length);

     for (var ct=last_index; ct<quan_st.length; ct++)
     {
         if (quan_st.substr(ct,1)==" ") { break; }
         user_qt1=user_qt1+(quan_st.substr(ct,1));
         user_sz1=user_sz1+(size_st.substr(ct,1));
     }

     var user_qt2=user_qt1+quan_c;
     var user_sz2=user_sz1+size_c;

     quan_st=quan_st.replace(user_qt1,user_qt2);
     size_st=size_st.replace(user_sz1,user_sz2);

     sessionStorage.setItem("quan_data",quan_st);
     sessionStorage.setItem("size_data",size_st);

     var size_in="s_"+num, quan_in="q_"+num;
     document.getElementById(size_in).innerHTML=size_c;
     document.getElementById(quan_in).innerHTML=quan_c;

     for (var k=lst; k<lst+36; k++)
     {
...
document.getElementById('count').innerHTML=i_count;
     ...
     { alert('Invalid quantity and/or size!'); }
    }
    $(document).ready(function(){ $('#lp_error').show(); });
    document.getElementById('lp_error').style.opacity="1";
    document.getElementById('lp_error').innerHTML="Item added to cart!";
    document.getElementById('lp_error').style.background="seagreen";
    setTimeout(function() 
    { 
      document.getElementById('lp_error').style.background="coral";
      $('#lp_error').hide(); 
    }, 2000);
